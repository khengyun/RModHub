...
RModHub predicts RNA modification sites as a web service, two ways: from a **nucleotide
sequence** (12 types with MultiRM, instant) or from **nanopore direct-RNA signal** (6 types
with DirectRM, as a background job). It is built for wet-lab users who do not code and
targets the *Nucleic Acids Research* Web Server Issue: no login, no cookies, no third-party
assets, sample data one click away, rich output plus CSV, licenses on the landing page.

| | Branch A — sequence | Branch B — nanopore signal |
|---|---|---|
| Input | pasted ACGU sequence (51–10,000 nt) or one FASTA record | `.pod5` + BAM from `dorado --emit-moves --reference` + reference FASTA + regions CSV |
| Model | [MultiRM](https://github.com/Tsedao/MultiRM) (Song *et al.*, Nat Commun 2021, MIT) | [DirectRM](https://github.com/yuxinPenny/DirectRM) (Zhang *et al.*, Nat Commun 2025, MIT), run **unmodified** |
| Types | Am, Cm, Gm, Um, m1A, m5C, m5U, m6A, m6Am, m7G, Psi, A-to-I | ac4C, m1A, m5C, m6A, m7G, Psi |
| Output | sites with empirical p-value < alpha | per-site modification rate with 95 % CI, plus per-read probabilities |
| Execution | synchronous, sub-second | asynchronous Celery job in a separate worker container (`worker/`) |
| Stack | `api` (FastAPI + torch CPU) + `web` (nginx + React) | + `postgres`, `redis`, `worker` (`--profile phase2`) |

Both branches emit the **same row schema** (see [Result schema](#result-schema)), so one results
table, one track view and one CSV writer serve both.

## Quick start

```bash
docker compose up --build                       # sequence branch only: web UI on http://localhost:8080, API on :8000
...
Both branches (adds Postgres, Redis and the DirectRM worker; the worker image is ~1.8 GB unpacked
and 0.57 GB to pull — `docker image ls` reports 2.48 GB on a containerd image store):

```bash
cp .env.example .env                            # set POSTGRES_PASSWORD (openssl rand -hex 24; no @ % $ or whitespace),
                                                # RMODHUB_IP_HASH_SECRET (openssl rand -hex 32) and COMPOSE_PROFILES=phase2
make phase2-check                               # validates the password and the HMAC key (the api container refuses a missing/dev key)
docker compose --profile phase2 up --build --wait   # or: make phase2-up (runs phase2-check, waits for health)
make phase2-smoke                               # synthetic sample job end to end against the RMODHUB_WEB_PORT of .env (8080)
```

Three `.env` lines switch the branch on together: `POSTGRES_PASSWORD` (`docker-compose.yml`
derives `DATABASE_URL` and `CELERY_BROKER_URL` from it, and the API enables the job routes only
when `DATABASE_URL` is set), `COMPOSE_PROFILES=phase2` (Compose then includes `postgres`, `redis`
and `worker` in every command, `down` included; `make` adds `--profile phase2` by itself whenever
the password is set) and `RMODHUB_IP_HASH_SECRET` (the api container exits with a clear message
when the branch is enabled without a real key; `RMODHUB_ALLOW_DEV_SECRET=1` overrides that for
local experiments). The web UI shows the **Nanopore signal** tab only when `GET /api/capabilities`
reports `signal: true`. Without the branch every `/api/jobs` and `/api/uploads` route answers
`503 {"detail": "The nanopore signal branch is not enabled on this server."}` and still
appears in `/docs`.
...
For the signal branch outside compose, point `DATABASE_URL` / `CELERY_BROKER_URL` at a
Postgres and a Redis of your own, set `RMODHUB_IP_HASH_SECRET` (outside the image the API only
warns about the development default; see `.env.example`), and start the worker from its own
project: `cd worker && uv sync --all-groups && uv run celery -A rmodhub_worker.celery_app worker -Q signal --concurrency=1 --prefetch-multiplier=1`.

Interactive API docs: <http://localhost:8000/docs> (also proxied at `:8080/docs`; Swagger UI is
self-hosted from `app/static/swagger/`, no CDN). The API also serves a minimal landing page
with the license notices at <http://localhost:8000/>.
...
```bash
uv run pytest                     # API + sequence branch + job API on SQLite + hardening + config + tools/sample tests (218 tests, ~15 s)
cd worker && uv run pytest        # worker: prepare, pipeline on the sample (three full runs), golden DirectRM comparison, subset equivalence, cancel, aggregate (117 tests, ~40 s)
cd frontend && npm test           # vitest (187 tests)
cd frontend && npm run e2e        # Playwright against E2E_BASE_URL (default http://localhost:8080)
make phase2-smoke                 # live stack: capabilities, sample job to `done`, results, CSV headers
```

Golden fixtures: `tests/fixtures/golden_multirm_151nt/` (unmodified MultiRM) and
`worker/tests/fixtures/golden_directrm_sample/` (unmodified DirectRM on the synthetic sample);
both suites require the served pipelines to reproduce them exactly.
...
- RModHub server, worker and web interface: **MIT** (`LICENSE`).
- MultiRM: MIT, © 2021 Zitao Song (`app/predictors/multirm/vendor/LICENSE`).
- DirectRM: MIT, © 2025 Yuxin Zhang (`worker/directrm_vendor/LICENSE`); scripts vendored
  byte-identical, weights re-serialised to CPU (see `worker/directrm_vendor/UPSTREAM.md`).
- k-mer level tables `5mer_levels_v1.txt` / `9mer_levels_v1.txt`: Oxford Nanopore Technologies
  `kmer_models`, **MPL-2.0**.
- Remora (`ont-remora`, a dependency of the worker image used for signal-to-reference alignment):
  **Oxford Nanopore Technologies Public License 1.0 — research use only.** The signal branch is
  therefore for non-commercial research use; this is stated on the landing page and on the signal tab.
- Swagger UI: Apache-2.0 (`app/static/swagger/LICENSE`).
- Synthetic sample data (`app/samples/signal/`): CC0 1.0.

## Citation

If you use the sequence branch, cite MultiRM:

> Song Z, Huang D, Song B, Chen K, Song Y, Liu G, Su J, de Magalhães JP, Rigden DJ, Meng J.
> Attention-based multi-label neural networks for integrated prediction and interpretation of
> twelve widely occurring RNA modifications. *Nature Communications* 12, 4011 (2021).
> <https://doi.org/10.1038/s41467-021-24313-3>

If you use the nanopore signal branch, cite DirectRM:

> Zhang Y, Wu Y, Ma J, *et al.* DirectRM: integrated detection of landscape and crosstalk between
> multiple RNA modifications using direct RNA sequencing. *Nature Communications* 16, 9450 (2025).
